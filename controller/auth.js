...
async function signUp(req,res){
     ...
     try{
	  ...
	  if(req.body.roles){
           ...
           const result = await user.setRoles([1])    //setRoles this mathod only come (many to many)
           console.log("default roles", result)
        }
      res.send({msg: 'User has beend created successfully'})
	...
	try{
		...
		if(user){
			...
			//const token = await jwt.sign({id : user.id}, process.env.JWT_SECRET_KEY, {
				const token = await jwt.sign({id : user.id}, 'helloIamsecretkye',{
				expiresIn: '1h'
			})
