...
async function filterBasedOnProduct(req,res){
	const CategoryId = req.query.CategoryId;  //?CategroyId=3
	const name = req.query.name;              //?name =
	const minCost = req.query.minCost;        //minCost=450         aguloke postman a giya (http://localhost:2500/ecom/api/v1/products?CategroyId=3)
	const maxCost = req.query.maxCost;        //maxCost=750          eirokom search korte hobe
	...
	if(minCost && maxCost){
		const result = await Products.findAll({
			where: {
				cost:{
					[Sequelize.Op.gte] : minCost,      //gte means -grater then
					[Sequelize.Op.lte] : maxCost       //lte means - less then
				}
			}
		})
		res.send(result)
